...
class MANIQA(nn.Module):
    def __init__(self, embed_dim=72, num_outputs=1, patch_size=8, drop=0.1, 
        ...
        # self.vit = timm.create_model('vit_tiny_patch16_224', pretrained=True) # ≈5.5M
        # self.vit = timm.create_model('vit_small_patch16_224', pretrained=True) # ≈22M
        self.vit = timm.create_model('vit_large_patch16_224', pretrained=True) # ≈307M
        ...
        self.slic_conv = nn.Conv2d(1, 4096, 1, 1, 0) # tiny 768 small 1536 large 4096
        
        # texture
        ######################################################################
        self.ta = TA(3072)
        self.t_conv = nn.Conv2d(3072, 4096, 1, 1, 0)
        ...
        self.fusionconv = nn.Conv2d(4096 * 2, embed_dim * 4, 1, 1, 0) # tiny embed_dim * 2 small emb_dim * 4 large 4096 * 2
